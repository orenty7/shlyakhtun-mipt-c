...
template<typename T>
void push_back(List<T> *list, T new_val) {
...
template<typename T>
void push_front(List<T> *list, T new_val) {
    Node<T> *new_node = new Node<T> {
	.value = new_val,
	.next = list->head
    };
...
template<typename T>
void insert(List<T> *list, T new_val, int index) {
...
template<typename T>
T remove(List<T> *list, int index) {
    if(index == 0) 
